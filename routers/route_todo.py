...
async def create_todo(request: Request, data: TodoBody, response: Response, csrf_protect: CsrfProtect = Depends()):
...
async def get_single_todo(request: Request, id: str, response: Response):
...
async def update_todo(request: Request, id: str, data: TodoBody, response: Response, csrf_protect: CsrfProtect = Depends()):
    new_token = auth.verify_csrf_update_jwt(
        request, csrf_protect, request.headers
    )
    todo = jsonable_encoder(data)
...
async def delete_todo(request: Request, id: str, response: Response, csrf_protect: CsrfProtect = Depends()):
    new_token = auth.verify_csrf_update_jwt(
        request, csrf_protect, request.headers
    )
